...
class BuildCommand extends Command
{
    ...
    protected function getEnvironmentVariables(): array
    {
        $variables = array_merge(
            [
                ...
                'NATIVEPHP_NOTARIZE' => config('nativephp-internal.notarization.enabled') ? '1' : '0',
        ...
        foreach ([
            ...
            'NATIVEPHP_MAC_IDENTITY' => config('nativephp-internal.mac_signing.identity'),
            'CSC_LINK' => config('nativephp-internal.mac_signing.csc_link'),
            'CSC_KEY_PASSWORD' => config('nativephp-internal.mac_signing.csc_key_password'),
            'APPLE_ID' => config('nativephp-internal.notarization.apple_id'),
            'APPLE_APP_SPECIFIC_PASSWORD' => config('nativephp-internal.notarization.apple_id_pass'),
            'APPLE_TEAM_ID' => config('nativephp-internal.notarization.apple_team_id'),
        ] as $key => $value) {
            if ($value !== null && $value !== '') {
                $variables[$key] = $value;
            }
    ...
    private function updateElectronDependencies(): void
    {
        ...
        $result = Process::path(__DIR__.'/../../resources/js/')
        ...
        if ($result->failed()) {
            throw new \RuntimeException('npm install failed while updating Electron dependencies.');
        }
    }
    ...
    private function buildOrPublish(): void
    {
        ...
        if ($result->failed()) {
            throw new \RuntimeException("npm run {$this->buildCommand}:{$this->buildOS} failed.");
        }
    }
}
