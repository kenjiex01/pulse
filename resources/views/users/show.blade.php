...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Detalye ng user account</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('users.edit', $user) }}" class="btn-secondary">I-edit</a>
            @can('delete', $user)
                <form method="POST" action="{{ route('users.destroy', $user) }}" onsubmit="return confirm('Sigurado ka bang buburahin ang user na ito?')">
                    @csrf
                    @method('DELETE')
    ...
    <div class="max-w-lg card-panel space-y-4">
        ...
        <div>
            <p class="text-sm text-gray-600">Ginawa noong</p>
