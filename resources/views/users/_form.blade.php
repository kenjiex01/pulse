...
<div>
    ...
    <select id="role_id" name="role_id" required class="form-input">
        <option value="">Pumili ng role</option>
...
<div>
    <label for="password" class="form-label">Password {{ $isEdit ? '(opsyonal)' : '' }}</label>
    <input id="password" name="password" type="password" {{ $isEdit ? '' : 'required' }} class="form-input">
    @error('password')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
</div>

<div>
    <label for="password_confirmation" class="form-label">Kumpirmahin ang Password</label>
