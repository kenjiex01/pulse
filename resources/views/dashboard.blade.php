...
@section('content')
    <div class="welcome-banner">
        ...
        <div class="relative">
            ...
            <h1 class="mb-1 text-2xl font-bold sm:text-3xl">Maligayang pagdating, {{ $user->name }}</h1>
            <p class="max-w-xl text-sm text-white/85 sm:text-base">
                ISKOLARIS desktop workspace — pamahalaan ang users, roles, at system access mula sa isang lugar.
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4 lg:gap-6">
        ...
        @include('partials.stat-card', [
            ...
            'subtitle' => $user->role?->slug ?? 'Walang role',
        ...
        @if ($user->isAdmin())
            @include('partials.stat-card', [
                'index' => 2,
                'title' => 'Kabuuang Users',
                'value' => $userCount,
                'subtitle' => 'Lahat ng account sa sistema',
            ...
            @include('partials.stat-card', [
                'index' => 3,
                'title' => 'Kabuuang Roles',
                'value' => $roleCount,
                'subtitle' => 'Mga naka-configure na access level',
        ...
        @else
            @include('partials.stat-card', [
                ...
                'title' => 'Status',
                'value' => 'Aktibo',
                'subtitle' => 'Matagumpay na naka-login',
                'progress' => 100,
            ...
            @include('partials.stat-card', [
                ...
                'title' => 'Access',
                'value' => 'Limitado',
                'subtitle' => 'Contact admin para sa higit pang modules',
                'progress' => 60,
    ...
    @if ($user->isAdmin())
        <div>
            <h2 class="mb-4 text-lg font-semibold text-gray-900">Mabilisang access</h2>
            <div class="grid gap-4 sm:grid-cols-2">
                <a href="{{ route('users.index') }}" class="group quick-link-card block no-underline">
                    ...
                    <p class="mb-3 mt-1 text-xs text-gray-500">Gumawa, i-edit, at pamahalaan ang mga user account.</p>
                ...
                <a href="{{ route('roles.index') }}" class="group quick-link-card block no-underline">
                    ...
                    <p class="mb-3 mt-1 text-xs text-gray-500">I-configure ang mga role at permission level.</p>
                    <span class="inline-flex items-center text-xs font-medium text-[#00A3E6]">
                        Buksan
                        <svg class="ml-1 h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                    </span>
