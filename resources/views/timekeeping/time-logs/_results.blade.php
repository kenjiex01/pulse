    ...
    <div class="overflow-x-auto">
        <table class="table-skolaris min-w-[720px]">
                ...
                <tr>
                    @can('time-logs.delete')
                        <th class="w-10 px-3 py-2">
                            <input type="checkbox" data-time-logs-select-all aria-label="Select all batches">
                        </th>
                    @endcan
            ...
            <tbody>
                @forelse ($records as $record)
                    <tr>
                        @can('time-logs.delete')
                            <td class="px-3 py-2">
                                <input
                                    type="checkbox"
                                    name="selected_ids[]"
                                    value="{{ $record->timekeeping_transaction_id }}"
                                    form="time-logs-purge-form"
                                    data-time-logs-row-select
                        ...
                        <td class="px-3 py-2 text-right">
                            <a
                                href="{{ route(TimeLogsSupport::routeName('tab'), ['tab' => $tab, 'view' => $record->timekeeping_transaction_id, 'search' => request('search')]) }}"
                                class="btn-icon"
                                title="View batch"
                                data-no-loader
                            >
                ...
                @empty
                    <tr>
                        <td colspan="{{ count($config['columns']) + (auth()->user()?->can('time-logs.delete') ? 2 : 1) }}" class="py-12 text-center text-sm text-gray-500">
