...
class EmployeeGovernmentIdValidationTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_create_rejects_invalid_sss_number(): void
    {
        $user = User::query()->firstOrFail();
        $payload = $this->employeePayload();
        $payload['sss_number'] = '12-345-678';
        ...
        $this->actingAs($user)
            ...
            ->assertSessionHasErrors('sss_number');
    }

    public function test_update_rejects_invalid_philhealth_number(): void
    {
        $user = User::query()->firstOrFail();
        $employee = $this->createEmployee($user);
        $payload = $this->employeePayload($employee);
        $payload['philhealth_number'] = '123456789';

        $this->actingAs($user)
            ->put(route('employees.update', $employee), $payload)
            ->assertSessionHasErrors('philhealth_number');
    }

    public function test_update_accepts_valid_government_numbers_without_dashes(): void
    {
        $user = User::query()->firstOrFail();
        $employee = $this->createEmployee($user);
        $payload = $this->employeePayload($employee);
        $payload['tin_number'] = '123456789';
        $payload['sss_number'] = '3412345678';
        $payload['philhealth_number'] = '123456789012';
        $payload['pagibig_number'] = '123456789012';
        ...
        $this->assertSame('123456789', $employee->tin_number);
        $this->assertSame('3412345678', $employee->sss_number);
    }

    public function test_update_strips_dashes_before_save(): void
    {
        $user = User::query()->firstOrFail();
        $employee = $this->createEmployee($user);
        $payload = $this->employeePayload($employee);
        $payload['sss_number'] = '34-1234567-8';
        $payload['tin_number'] = '123-456-789';
    ...
    private function employeePayload(?Employee $employee = null): array
    {
        ...
        return [
            ...
            'email' => $employee?->email ?? 'gov-id.test@example.com',
