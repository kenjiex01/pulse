...
./install-pulse-mac.sh ~/Downloads/Pulse.app
```
...
2. Double-click **Install Pulse.command** (if blocked: right-click → Open).
3. **First launch:** Applications → right-click Pulse → Open → Open.
...
Local/testing builds without these vars use adhoc sign via `./scripts/fix-mac-pulse-codesign.sh` during `build-desktop.sh`. DMG includes **Install Pulse.command** for one-click client install (still requires first right-click Open — Gatekeeper limit on unsigned apps).

### Free vs paid

| Approach | Cost | Client experience |
|----------|------|-------------------|
| **Unsigned + install helper** | Free | One-time right-click Open; use `Install Pulse.command` in DMG |
